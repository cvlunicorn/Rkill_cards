game.import("extension", function (lib, game, ui, get, ai, _status) {
    return {
        ...
        content: function (config, pack) {
                        ...
                        "changshecheng9": {
                            ...
                            ai: {
                                ...
                                equipValue: function (card, player) {
                                    ...
                                    var result = function () {
                                        if (!game.hasPlayer(function (current) {
                                            return get.distance(player, current) <= 2 && player.canUse('sha', current) && get.effect(current, { name: 'sha' }, player, player) > 0;
                                        })) {
                                            return 1;
                                        }
                                        if (player.hasSha() && _status.currentPhase == player) {
                    ...
                    skill: {
                        ...
                        duanbing_R: {
                            ...
                            content: function () {
                                "step 0";
                                player
                                    .chooseTarget(get.prompt("duanbing_R"), "为" + get.translation(trigger.card) + "增加一个目标", function (card, player, target) {
                                ...
                                "step 2";
                                player.logSkill("duanbing_R", event.target);
                                ...
                                effect: {
                                    player: function (card, player, target, current, isLink) {
                                        if (!isLink && card.name == "sha") {
                                            if (player._duanbing_Rtmp) return;
                                            player._duanbing_Rtmp = true;
                                            if (get.effect(target, card, player, player) <= 0) {
                                                delete player._duanbing_Rtmp;
                                                return;
                                            }
                                            if (
            ...
            lib.translate['jianrmhbao_card_config'] = '舰R美化卡包';
            lib.config.all.cards.push('jianrmhbao');
            //if (!lib.config.cards.contains('jianrmhbao')) lib.config.cards.push('jianrmhbao');//包名翻译

            //选项触发内容，原因见config
        }, help: {}, config: { /* "jianrmh": { "name": "舰R美化", "init": true } */ }, package: {
            character: {
                character: {
                },
                translate: {
